 ...
 *
 */
class CanvassRoom(context: Context, private var xmlparser: XMLActivity): View(context), View.OnTouchListener, TextToSpeech.OnInitListener {
    ...
    private fun drawBoundaries(canvas: Canvas){
        ...
        //Value for scaling
        val resizeVal = heightScreen/60




        // Debug for the dimension of the screen
        //Log.e("width", String.format("%s %d %f %d %f", "Width and height", widthScreen,axmin,heightScreen,aymax))
                ...
                polygons.forEach {
                    if (it.contains(PointF(x, y))) {
                        if (!cell ){
                            ...
                            vibe.cancel()
                            tts.speak(it.name, TextToSpeech.QUEUE_FLUSH, null,"")
